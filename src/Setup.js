...
// MOONBASE ALPHA
// const ordersManager_Moonbeam_address = "0x1374443BA8451ab11D5778C08dB7b32b92B945AF";
// const usersRegistry_Moonbeam_address = "0x70388FDc83f5D1f69Dd6595112366C0e578CcEe5";
// const executionOrdersEngine_Moonbeam_address = "0x05c8ff63cC0dd09B3043b055f5B21B6D5E2Ed588";
// MOONBEAM
...
const axelar_fantom_fee = "1";   // 1 FTM Todo
...
const setChainOfUser = (_userChain) => {
	userChain = _userChain;       //  MOONBEAM,FANTOM,    BINANCE,BINANCETESTNET,MOONBASE,ASTAR,INTERLAY
...
const setWallet = (_wallet=null, _mm_chainId=null, _mm_account=null) => { 
	if (_wallet) {
		...
		console.log(`mm_account: ${mm_account} New wallet : `,wallet,` ***** mm_chainId: ${mm_chainId}`); //Moonbeam=1284 , Moonriver=1285, MoonbaseAlpha=1287
...
const get_StellaSwap_Prices = async () => {
	...
	// const price_WGLMR_USDC = Number(1 / (ethers.utils.formatUnits(`${wglmr_USDC}`, 18))).toFixed(5);
	// const price_WGLMR_USDC = wglmr_USDC;

	const astr_USDC  = await OrdersManager_instance.priceUSDC(xcASTR_address);
	const price_ASTR_USDC =(astr_USDC / 1000000).toFixed(5);
	// const price_ASTR_USDC = Number(1 / (ethers.utils.formatUnits(`${astr_USDC}`, 18))).toFixed(5);
	// const price_ASTR_USDC = astr_USDC;
...
const deleteOrder_Moonbeam = async (orderNonce,_size,tokenIn) => {
	...
	return new Promise (async (resolve,reject) => {
		...
		if (userChain==="MOONBEAM")
		{
			...
			// 	console.log(`********************* OrdersManager_instance DELETE ORDER End is mined orderNonce: ${orderNonce} *********************************** reslveMsg: `,reslveMsg);
			// 	resolve(reslveMsg);
			// });
...
// const set_artifical_StellaSwapPrice = async (Dot_Price, WGLMR_Price, Astr_Price ) => {
//     console.log(`********************* OrdersManager_instance Set Artifical Prices Start ***********************************`);
//     const tx_set_artifical_StellaSwapPrice =  await OrdersManager_instance.set_artifical_StellaSwapPrice( Dot_Price, WGLMR_Price, Astr_Price );
